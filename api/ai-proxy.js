module.exports = async function handler(req, res) {
  ...
  try {
    const { provider, key, model, action, system, user, json, baseUrl, maxTokens } = req.body || {};
    ...
    if (!effectiveKey || effectiveKey.length > 5000) {
      return res.status(400).json({ error: `Chave de API ausente. Informe uma chave no app ou configure ${provider === 'openai' ? 'OPENAI_API_KEY' : 'GEMINI_API_KEY'} no Vercel.` });
    }
    ...
    if (action === 'models') {
      ...
      return res.status(200).json({ models: (body.data || []).map(m => m.id).filter(Boolean) });
    }

    const chosen = String(model || '').trim();
    if (!chosen) return res.status(400).json({ error: 'Modelo ausente.' });
    ...
    try {
      body = await providerFetch(`${base}/chat/completions`, {
        method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${effectiveKey}` }, body: JSON.stringify(requestBody)
      });
    } catch (error) {
    ...
    return res.status(status).json({ error: error?.name === 'AbortError' ? 'A solicitação à IA excedeu o tempo limite.' : (error.message || 'Erro no proxy de IA.') });
  }
};
