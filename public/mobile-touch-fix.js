(()=>{
  'use strict';
  if(!window.__MapaFlexTouchBridgeInstalled){
  ...
  const load=(src,key)=>{
    if(document.querySelector(`script[data-mf-${key}]`)) return;
    const s=document.createElement('script');s.src=src;s.defer=true;s.dataset[`mf${key[0].toUpperCase()+key.slice(1)}`]='1';document.head.appendChild(s);
  };
  load('/mobile-ui-hotfix.js','hotfix');
  load('/presentation-ux.js','presentation');
})();
