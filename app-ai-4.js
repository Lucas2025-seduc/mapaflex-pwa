  ...
  // O conteúdo protegido é cifrado com AES-GCM; a chave é derivada da senha por PBKDF2/SHA-256.
  ...
  function mfProtectedPayload(n){
    return {
      text:n.text||'',note:n.note||'',link:n.link||'',linkTitle:n.linkTitle||'',image:n.image||'',
      attachments:Array.isArray(n.attachments)?n.attachments:[],bold:!!n.bold,italic:!!n.italic,
      shape:n.shape||'rounded',color:n.color||'#ffffff',presentOrder:n.presentOrder||''
    };
  }
  async function mfEncryptNodePayload(n,password){
    const salt=crypto.getRandomValues(new Uint8Array(16));
    const iv=crypto.getRandomValues(new Uint8Array(12));
    const key=await mfPrivacyKey(password,salt);
    const plain=mfPrivacyEncoder.encode(JSON.stringify(mfProtectedPayload(n)));
    const encrypted=new Uint8Array(await crypto.subtle.encrypt({name:'AES-GCM',iv},key,plain));
    return {version:1,locked:true,kdf:'PBKDF2-SHA256',cipher:'AES-256-GCM',iterations:MF_PRIVACY_ITERATIONS,salt:mfBytesToB64(salt),iv:mfBytesToB64(iv),ciphertext:mfBytesToB64(encrypted),lockedAt:new Date().toISOString()};
  }
  async function mfDecryptNodePayload(n,password){
    ...
    const salt=mfB64ToBytes(p.salt),iv=mfB64ToBytes(p.iv),data=mfB64ToBytes(p.ciphertext);
    const key=await mfPrivacyKey(password,salt,Number(p.iterations)||MF_PRIVACY_ITERATIONS);
    const plain=await crypto.subtle.decrypt({name:'AES-GCM',iv},key,data);
    return JSON.parse(mfPrivacyDecoder.decode(plain));
  }
  function mfClearNodePlaintext(n){
    n.text='🔒 Balão protegido';n.note='';n.link='';n.linkTitle='';n.image='';n.attachments=[];
    n.bold=false;n.italic=false;n.presentOrder='';n.shape='rounded';n.color='#f1f5f9';
  }
  async function mfProtectSelectedNode(){
    ...
    const password=prompt('Crie uma senha para ocultar este balão. Use pelo menos 8 caracteres.');
    if(password===null)return;
    if(password.length<8){alert('Use uma senha com pelo menos 8 caracteres.');return;}
    const confirmation=prompt('Confirme a senha do balão. Não existe recuperação automática se ela for esquecida.');
    if(confirmation===null)return;
    if(password!==confirmation){alert('As senhas não coincidem.');return;}
    try{
      const protection=await mfEncryptNodePayload(n,password);
      n.protection=protection;
      mfClearNodePlaintext(n);
      history=[];future=[];editingNode=null;hideNodeToolbar();
      const parent=n.parent?nodeById(n.parent):null;
      selected=parent&&!mfIsProtectedNode(parent)?parent:(state.nodes.find(x=>!mfIsProtectedNode(x))?.id||null);
      render();saveLocal();mfUpdatePrivacyUi();
      alert('Balão ocultado e protegido. Para vê-lo novamente será necessário informar a senha.');
  ...
  async function mfUnlockNode(id){
    ...
    const password=prompt('Digite a senha para desocultar este balão:');
    if(password===null)return;
    try{
      const payload=await mfDecryptNodePayload(n,password);
      Object.assign(n,payload);
      delete n.protection;
      history=[];future=[];selected=n.id;editingNode=null;
      render();saveLocal();mfCloseProtectedManager();mfUpdatePrivacyUi();
    }catch(err){console.error(err);alert('Senha incorreta ou conteúdo protegido inválido.');}
  }

  const mfBaseVisibleNodes=visibleNodes;
  visibleNodes=function(){return mfBaseVisibleNodes().filter(n=>!mfIsProtectedNode(n));};
  ...
  render=function(){
    if(selected&&mfIsProtectedNode(nodeById(selected)))selected=state.nodes.find(n=>!mfIsProtectedNode(n))?.id||null;
    mfBaseRender();mfUpdatePrivacyUi();
  };

  function mfProtectedNodes(){return state.nodes.filter(mfIsProtectedNode);}
  function mfUpdatePrivacyUi(){
    const count=mfProtectedNodes().length;
    const countEl=$('mfProtectedCount');if(countEl)countEl.textContent=String(count);
    const manager=$('mfProtectedManagerBtn');if(manager)manager.innerHTML=`🔐 <strong>Protegidos</strong><small>${count} oculto${count===1?'':'s'}</small>`;
    const protect=$('mfProtectNodeBtn');if(protect)protect.disabled=!selected||mfIsProtectedNode(nodeById(selected));
    const quick=$('mfQuickExplainBtn');if(quick)quick.disabled=!selected||mfIsProtectedNode(nodeById(selected));
    const status=$('status');
    if(status&&count&&!/protegido/i.test(status.textContent))status.textContent+=` · ${count} protegido${count===1?'':'s'}`;
  }
  function mfOpenProtectedManager(){
    ...
    else list.innerHTML=nodes.map((n,i)=>{
      let when='';try{when=n.protection?.lockedAt?new Date(n.protection.lockedAt).toLocaleString('pt-BR'):'';}catch(e){}
      return `<div class="mfPrivacyItem"><div><b>🔒 Balão protegido ${i+1}</b><small>${when?`Protegido em ${escapeHtml(when)}`:'Conteúdo cifrado'}</small></div><button class="btn primary" data-mf-unlock="${escapeHtml(n.id)}">Desocultar</button></div>`;
    }).join('');
  ...
  async function mfQuickExplainSelected(){
    const n=nodeById(selected);if(!n||mfIsProtectedNode(n)){alert('Selecione um balão visível para explicar.');return;}
    ...
    try{
      ...
      const txt=await callAi(prompt,false,2600);
      setAiStatus('Explicação pronta.','ok');openAiResult(txt,'explain',null);
  ...
  function mfInstallPrivacyUi(){
    ...
    if(sidebar){
      const sec=document.createElement('div');sec.className='section';sec.innerHTML='<h3>Privacidade</h3><div class="toolgrid"><button class="tool" id="mfProtectedManagerBtn">🔐 <strong>Protegidos</strong><small><span id="mfProtectedCount">0</span> ocultos</small></button><button class="tool" id="mfProtectSelectedSide">🔒 <strong>Ocultar</strong><small>Com senha</small></button></div>';
      const help=sidebar.querySelector('.section.help');sidebar.insertBefore(sec,help||null);
    }
    const form=$('nodeForm');
    if(form){
      const field=document.createElement('div');field.className='field';field.id='mfNodePrivacyField';field.innerHTML='<div class="hr"></div><label>IA e privacidade</label><div class="mfPrivacyActions"><button class="btn primary" id="mfQuickExplainBtn">✨ Explicar com IA</button><button class="btn" id="mfProtectNodeBtn">🔒 Ocultar com senha</button></div><div class="help" style="margin-top:6px">Ao proteger, o conteúdo do balão é cifrado e o balão some do mapa. A senha será exigida para desocultá-lo.</div>';
      form.appendChild(field);
    }
    const toolbar=$('nodeToolbar');
    if(toolbar&&!$('tbAiExplain')){const b=document.createElement('button');b.id='tbAiExplain';b.title='Explicar este conceito com IA';b.textContent='✨ IA';toolbar.insertBefore(b,$('tbDone'));}
    const ctx=$('contextMenu');
    if(ctx&&!$('ctxAiExplain')){const b=document.createElement('button');b.id='ctxAiExplain';b.textContent='✨ Explicar com IA';ctx.insertBefore(b,$('ctxDelete'));}

    const modal=document.createElement('div');modal.id='mfProtectedModal';modal.className='mfPrivacyModal';modal.setAttribute('aria-hidden','true');modal.innerHTML='<div class="mfPrivacyCard" role="dialog" aria-modal="true" aria-labelledby="mfPrivacyTitle"><div class="mfPrivacyHead"><div><b id="mfPrivacyTitle">Balões protegidos</b><div class="help">A senha correta é necessária para restaurar o conteúdo.</div></div><button class="btn" id="mfPrivacyClose">Fechar</button></div><div class="mfPrivacyBody" id="mfProtectedList"></div></div>';
    ...
    $('mfProtectedManagerBtn')?.addEventListener('click',mfOpenProtectedManager);
    $('mfQuickExplainBtn')?.addEventListener('click',mfQuickExplainSelected);
    $('tbAiExplain')?.addEventListener('click',()=>{finishInlineEdit();mfQuickExplainSelected();});
    $('ctxAiExplain')?.addEventListener('click',()=>{hideContext();mfQuickExplainSelected();});
    $('mfPrivacyClose')?.addEventListener('click',mfCloseProtectedManager);
    modal.addEventListener('click',e=>{if(e.target===modal)mfCloseProtectedManager();});
    $('mfProtectedList')?.addEventListener('click',e=>{const btn=e.target.closest('[data-mf-unlock]');if(btn)mfUnlockNode(btn.dataset.mfUnlock);});
    document.addEventListener('keydown',e=>{if(e.key==='Escape'&&modal.classList.contains('visible'))mfCloseProtectedManager();});
    mfUpdatePrivacyUi();
  }
