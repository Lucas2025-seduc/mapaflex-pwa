...
async function upsertLicense(sql, { userId, customerId, subscriptionId, status = 'active', periodStart = null, periodEnd = null, priceId = null, cancelAtPeriodEnd = false, canceledAt = null }) {
  ...
  const [sub] = await sql`
    insert into mapaflex.subscriptions(
      ...
      ${userId}, 'pro', ${customerId}, ${subscriptionId}, ${priceId}, ${status},
      ${periodStart}, ${periodEnd}, ${!!cancelAtPeriodEnd}, ${canceledAt}, now()
    )
    on conflict(stripe_subscription_id) do update set
      ...
      current_period_start = coalesce(excluded.current_period_start, mapaflex.subscriptions.current_period_start),
      current_period_end = coalesce(excluded.current_period_end, mapaflex.subscriptions.current_period_end),
      cancel_at_period_end = excluded.cancel_at_period_end,
      canceled_at = coalesce(excluded.canceled_at, mapaflex.subscriptions.canceled_at),
      updated_at = now()
    returning id
  `;

  const nextStatus = licenseStatus(status);
  const graceUntil = nextStatus === 'grace' ? new Date(Date.now() + 7 * 86400000).toISOString() : null;
  let [license] = await sql`select id from mapaflex.licenses where subscription_id = ${sub.id} limit 1`;

  if (license) {
    [license] = await sql`
      update mapaflex.licenses
      set auth_user_id = ${userId}, plan_code = 'pro', status = ${nextStatus},
    ...
    [license] = await sql`
      ...
      values(${userId}, ${sub.id}, 'pro', ${nextStatus}, 2, ${periodEnd}, ${graceUntil})
      returning id
    `;
  }

  const enabled = nextStatus === 'active' || nextStatus === 'grace';
...
export async function onRequest(context) {
  ...
  try {
    ...
    if (!event?.id || !event?.type) return reply(400, { error: 'Evento inválido.' });
    ...
    try {
      ...
      if (event.type === 'checkout.session.completed') {
        ...
        if (userId && customerId && subscriptionId && paid) {
      ...
      if (event.type.startsWith('customer.subscription.')) {
        ...
        await upsertLicense(sql, {
          ...
          periodStart: iso(object.current_period_start || item?.current_period_start),
